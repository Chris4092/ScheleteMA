...
class _Section1State extends State<Section1> {
  ...
  late Future<List<Entity>> entityLoadFunction;

  Future<List<Entity>> initialiseEntitiesToLoad() async {
    List<Entity> entities = await repo.getAll();
    if(entityToAdd!=null) {
  ...
  void initStateAfterAdd(Entity entity) {
    entityToAdd = entity;
    initState();
    entityToAdd = null;
  }
  ...
  FutureBuilder<List<Entity>> getAsyncEntities(){
    return FutureBuilder<List<Entity>>(builder: (BuildContext ctxt, AsyncSnapshot<List<Entity>> snapshot) {
  ...
  ListView getEntityListView(List<Entity> entities){
        ...
        itemBuilder: (BuildContext ctxt, int index) {
          return GestureDetector(
            ...
            child: Card(
              ...
              child: Column(
                children: [
                  ListTile(title: Text(entities[index].name),subtitle: Text(entities[index].status + ' ' + entities[index].intval.toString()),trailing:
                  GestureDetector(
                    onTap: () => {
